...
## Nuxt Server Integration Specifics

These rules apply when running the NestJS application within a Nuxt Monorepo or utilizing a custom server integration approach.

Custom Server Integration: The NestJS application should be hosted via the Nuxt Custom Server layer or run on a separate port and exposed via proxy.

Proxy Configuration: The front-end (Vue/Nuxt) environment must proxy all API calls (e.g., requests to /api/\*\*) to the dedicated NestJS server endpoint.

Session/Authentication:

Authentication logic (JWT, Session, OAuth) must be entirely managed by the NestJS layer.

The frontend must securely consume the authentication state. Use Nuxt Server Middleware or Hooks to pre-check the authentication status during the page load lifecycle.

Shared Types: DTOs and response interface types must be shared between the NestJS and Nuxt projects (e.g., via a dedicated shared package or types/ directory) to maximize Type Safety across the stack.
